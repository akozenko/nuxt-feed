<template>
  <div class="flex flex-col w-full h-full items-center p-4">
    ...
    <PostList v-else class="grow overflow-auto" :posts="posts" />

    <PostAddNew v-if="userId" />
    <RegistrationBlock v-else @registered="handleRegisteredUserEvent" />
  </div>

  <LazyPostSubscriber @liked-post="handleLikedPostEvent" @added-post="handleAddedNewPostEvent" />
</template>
